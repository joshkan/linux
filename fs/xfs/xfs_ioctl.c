	...
	void __user		*arg)
{
	...
	/*
	 * Regular files only.  A stream on a directory would mean nothing
	 * unless what is created in it inherited the stream, and inheritance is
	 * a feature of its own.  FMODE_WRITE above already refuses directories,
	 * but as a side effect rather than a statement of intent.
	 */
	if (!S_ISREG(VFS_I(ip)->i_mode))
		return -EINVAL;

	/*
	 * Where a file's data lands is a property of the file, so require what
	 * fcntl_set_rw_hint() requires for the hint.  Clearing needs no stream
	 * fd, so without this an unrelated writer could change another
